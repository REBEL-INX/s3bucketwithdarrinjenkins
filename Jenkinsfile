pipeline {
    ...
    environment {
        ...
        JIRA_SITE = "https://xavmil589.atlassian.net"
        JIRA_PROJECT = "jiraandjenkins"
    }
    ...
    stages {
        ...
        stage('Checkout Code') {
            steps {
                git branch: 'main', url: 'https://github.com/REBEL-INX/s3bucketwithdarrinjenkins.git'
            }
        }
                ...
                script {
                    withCredentials([string(credentialsId: 'SONARQUBE_TOKEN_ID', variable: 'SONAR_TOKEN')]) {
                        ...
                        def scanStatus = sh(script: '''
                            ...
                            -Dsonar.projectKey=REBEL-INX_s3bucketwithdarrinjenkins \
                            -Dsonar.organization=rebel-inx \
                        ...
                        if (scanStatus != 0) {
                            ...
                            def sonarIssues = sh(script: '''
                                ...
                                "${SONARQUBE_URL}/api/issues/search?componentKeys=REBEL-INX_s3bucketwithdarrinjenkins&severities=BLOCKER,CRITICAL&statuses=OPEN" | jq -r '.issues[].message' || echo "No issues found"
    ...
    script {
        withCredentials([
            ...
            withEnv(["JIRA_CREDS=${JIRA_USER}:${JIRA_TOKEN}"]) {
                ...
                def searchQuery = URLEncoder.encode("project=jiraandjenkins AND summary~\"${issueTitle}\" AND status != Done", "UTF-8")
                def searchResponse = sh(script: '''
                    export JIRA_CREDS
                    curl -s -u "$JIRA_CREDS" \
                    -X GET "https://xavmil589.atlassian.net/rest/api/3/search?jql=''' + searchQuery + '''" \
                ...
                def createResponse = sh(script: '''
                    export JIRA_CREDS
                    curl -X POST "https://xavmil589.atlassian.net/rest/api/3/issue" \
                    -u "$JIRA_CREDS" \
